...
bool GostCipher::configureContext(SSL_CTX* ctx) {
    // Имя провайдера проверь в системе: часто он называется "gost"
    ...
    // Ограничимся TLS 1.3 для чистоты
    SSL_CTX_set_min_proto_version(ctx, TLS1_3_VERSION);
    SSL_CTX_set_max_proto_version(ctx, TLS1_3_VERSION);

    // На время отладки — security level 0 (может резать ГОСТ на дефолтном уровне 2)
    ...
    // При необходимости можно явно задать sigalgs/groups под ГОСТ:
    // SSL_CTX_set1_sigalgs_list(ctx, "gostr34102012_256:gostr34102012_512");
    // SSL_CTX_set1_groups_list(ctx, "GOSTGRP256:GOSTGRP512");
